...
describe NNQ::REQ do
  ...
  it "cancels an outstanding request when a new one is sent" do
    Sync do |task|
      rep = NNQ::REP.bind("tcp://127.0.0.1:0")
      req = NNQ::REQ.connect(rep.last_endpoint)
  ...
  it "discards a pending request when REP calls receive again" do
    Sync do |task|
      rep  = NNQ::REP.bind("tcp://127.0.0.1:0")
  ...
  it "raises when send_reply is called without a pending request" do
    Sync do
      rep = NNQ::REP.bind("tcp://127.0.0.1:0")
